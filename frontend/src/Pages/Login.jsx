...
import LoginImage from "../assets/login.webp"; // adjust path
...
const Login = () => {
  ...
  // Handle redirect from query param
  ...
  const imageVariants = {
    hidden: { opacity: 0, x: 50 },
    visible: { opacity: 1, x: 0, transition: { type: "spring", delay: 2.4, stiffness: 80 } },
  };
    ...
    <div className="min-h-screen flex items-center py-4 justify-center px-4 sm:px-6 lg:px-8 bg-gray-100">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2">
          {/* Form Section */}
          <motion.div className="p-6 sm:p-10 md:p-16" variants={containerVariants} initial="hidden" animate="visible">
            <div className="mb-6 text-center">
              <Link to="/" className="text-3xl font-bold text-blue-600 tracking-tight">Shopy</Link>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5" noValidate>
              ...
              {error && (
                <motion.p className="text-red-600 text-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.1 }}>
                  {error}
                </motion.p>
              )}

              <motion.div className="flex items-center justify-between text-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.2 }}>
            ...
            <motion.p className="mt-6 text-sm text-center text-gray-600" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 2.6 }}>
          ...
          <motion.div className="hidden md:block relative" variants={imageVariants} initial="hidden" animate="visible">
            <img src={LoginImage} alt="E-commerce Promotion" className="object-cover w-full h-full" />
            <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-blue-900 opacity-60"></div>
            <div className="absolute bottom-8 left-8 text-white max-w-xs">
              <motion.h3 className="text-xl font-bold mb-1 tracking-tight" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.8 }}>
